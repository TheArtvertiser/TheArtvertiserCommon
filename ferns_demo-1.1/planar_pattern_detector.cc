...
bool KeyPointComparator(const cv::KeyPoint & k1, const cv::KeyPoint & k2 ){
...
#ifdef USE_FAST
  static int frame = 0;
  if(frame%60==0) fast = new cv::DynamicAdaptedFeatureDetector(new cv::FastAdjuster(20,true),100, maximum_number_of_points_to_detect, 10);
  fast->detect(input_image,keypointsFast);

  std::sort(keypointsFast.begin(),keypointsFast.end(),KeyPointComparator);

  for(int i=0;i<(int)keypointsFast.size();i++){
	  detected_points[i].u = keypointsFast[i].pt.x;
	  detected_points[i].v = keypointsFast[i].pt.y;
	  detected_points[i].score = keypointsFast[i].response;
	  detected_points[i].orientation_in_radians = keypointsFast[i].angle;
  }

  number_of_detected_points = (int)keypointsFast.size();
#endif
