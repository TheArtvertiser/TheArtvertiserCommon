...
void fine_gaussian_pyramid::compute_from_level0(void)
{
  ...
  case yape_pyramid_3:
    for(int i = 0; i < number_of_octaves; i++) {
      cvSmooth(aztec_pyramid[4 * i], aztec_pyramid[4 * i + 1], CV_BLUR, 3, 3);
      if (i < number_of_octaves - 1) cvPyrDown(aztec_pyramid[4 * i], aztec_pyramid[4 * i + 4]);
    }
    break;
  case yape_pyramid_5:
    for(int i = 0; i < number_of_octaves; i++) {
      cvSmooth(aztec_pyramid[4 * i], aztec_pyramid[4 * i + 2], CV_BLUR, 5, 5);
      if (i < number_of_octaves - 1) cvPyrDown(aztec_pyramid[4 * i], aztec_pyramid[4 * i + 4]);
    }
    break;
  case yape_pyramid_7:
...
#else
      cvSmooth(aztec_pyramid[4 * i], aztec_pyramid[4 * i + 3], CV_GAUSSIAN, 7, 7);
#endif
      if (i < number_of_octaves - 1) cvPyrDown(aztec_pyramid[4 * i], aztec_pyramid[4 * i + 4]);
    }
    break;
  case full_pyramid_333: {
    ...
    for(int i = 0; i < number_of_octaves; i++) {
      ...
      cvSmooth(aztec_pyramid[4 * i + 1], aztec_pyramid[4 * i + 2], CV_GAUSSIAN, 3, 3);
      cvSmooth(aztec_pyramid[4 * i + 2], aztec_pyramid[4 * i + 3], CV_GAUSSIAN, 5, 5);
#endif
